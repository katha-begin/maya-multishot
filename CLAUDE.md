...
**Add Shots project picker (2026-09-15):** `ui/add_shot_dialog.py` has a Project combo filled from `core/shot_discovery.py` (`list_projects`, `get_scene_base`, `discover_shots`) -- `ctx_config.json` folded into the project it points at, roots for the running platform, the scene's project selected. Only the selected project is scanned and listed (Episode / Sequence / Shot); picking another project lists that one. A project it cannot list shows a grey line with the reason (config error / scene folder not found / no shots) instead of a blank tree. Existing-shot marking applies only to the scene's project. Tree building runs with signals blocked, and unchecked items reset colours by clearing `BackgroundRole`/`ForegroundRole` (`QtCore.Qt.Transparent` does not exist -- it raised on every uncheck).
...
**Set Shot memory / Script Editor flood (2026-09-15) -- do not revert:**
- `NodeManager._swap_reference`: never call `cmds.file(path, loadReference=...)` when the reference already loads that path -- Maya unloads and re-reads it anyway, so every shot switch reloaded every Alembic cache (ran out of memory on EGA scenes once the EGA config made their paths resolve). Never swap to a path that does not exist -- Maya leaves the reference unloaded and pointing at the missing file. Standin attrs are only set when the value changes.
- `DisplayLayerManager._resolve_top_node`: a reference `targetNode` resolves through the reference's own namespace, never `referenceQuery(nodes=True)` over the whole reference.
- `_MayaOutputHandler` hands records from non-main threads to `maya.utils.executeDeferred` (MGlobal off the main thread can crash Maya; the batch dispatcher logs from job threads). `executeDeferred` is looked up with `getattr` -- `maya.utils` can exist without it.
