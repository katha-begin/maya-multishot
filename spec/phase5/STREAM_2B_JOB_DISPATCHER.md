# Stream 2B — Job Dispatcher

**Status:** Not Started
**Round:** 2 (parallel with 2A)
**Branch:** `feature/batch-render`
**Dependencies:** Stream 1A (RenderJob, GPUInfo, JobStatus)

---

## Goal

Launch `Render` subprocesses per job per render layer, with one GPU semaphore
per available GPU. Monitor process completion, capture logs, invoke progress callback.

---

## `core/batch/job_dispatcher.py`
...
## Tests — `tests/test_job_dispatcher.py`

```python
# test_find_render_bin_env_var — MAYA_RENDER_BIN env var used
# test_dispatcher_init — semaphores created per GPU
# test_get_log_path — log path includes shot_id and layer
# test_notify_callback_called — on_progress invoked
# test_notify_callback_exception_safe — callback exception does not crash
# test_run_job_no_gpus — handles empty gpu_list gracefully
```
...
- [ ] `core/batch/job_dispatcher.py` created
- [ ] One `threading.Semaphore(1)` per GPU
- [ ] `dispatch(job)` is non-blocking
- [ ] `wait_all()` blocks until all threads done
- [ ] Per-layer subprocess loop with `-rl` flag
- [ ] `REDSHIFT_GPUDEVICES` env var set per process
- [ ] Unique `MAYA_APP_DIR` per GPU slot
- [ ] Log file captured per job per layer
- [ ] Progress callback invoked at each state change
- [ ] Tests pass
